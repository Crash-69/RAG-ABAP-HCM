...
FINAL_K = 15
BATCH_SIZE = 500
OUTPUT_DIR = Path(r"C:\Progetto_AI\RAG_V5_5_RESULTS")
CACHE_FILE = OUTPUT_DIR / "exact_cache_v5_5.pkl"
CACHE_VERSION = "5.5"
...
def split_abap_statements(text):
    """Split on ABAP statement periods, preserving useful SQL statements.
    This is intentionally conservative: it does not try to be a full ABAP parser.
    """
    text = text or ""
    text = text.replace("\r", " ").replace("\n", " ")
    parts = re.split(r"(?<=\.)\s+", text)
    return [clean_sql_fragment(x) for x in parts if x.strip()]
...
def extract_sql_relations(text):
    """Extract SELECT/FROM/WHERE relations one ABAP statement at a time.

    Important design rule: SELECT, FROM and WHERE must belong to the SAME
    statement. This prevents V5.4's false positives caused by linking unrelated
    terms from distant parts of a chunk.
    """
    relations = []
    for stmt in split_abap_statements(text):
        ...
        wm = re.search(r"\bWHERE\s+(.*)$", stmt, re.I)
        where = wm.group(1) if wm else ""
        fields_where = _fields_from_where(where) if where else []

        # Keep table-qualified fields too, e.g. PA0001-KOSTL.
        qualified = [f.upper() for f in re.findall(rf"\b{re.escape(table)}-([A-Z][A-Z0-9_]*)\b", stmt, re.I)]
...
def relationship_score(c, p):
    """V5.5: score only relationships proven inside one SQL statement.

    Co-presence of terms in a chunk is intentionally weak. Strong scores require
    FROM target table and/or requested fields in SELECT/WHERE of the same SQL.
    """
    text = c.upper()
    ...
    def consider(s, typ, reason):
        ...
        if s > score:
            score = s
        rank = {"NONE":0,"TABLE_FROM":1,"FIELD_FROM":2,"SELECT_FROM":3,"SELECT_FIELD_FROM":4,"SELECT_FIELD_WHERE":5,"SELECT_FIELDS_FROM":6,"SELECT_FIELDS_WHERE":7}.get(typ, 1)
        if rank > best_rank or (rank == best_rank and s > 0 and not best_reason):
            best_rank = rank
            best_type = typ
            best_reason = reason

    # Explicit target table query.
    for r in relations:
        table = r["table"]
        sel = set(r["fields_select"])
        wh = set(r["fields_where"])
        relevant = [f for f in requested if f in sel or f in wh]
        if table in target_tables:
            if relevant:
                s = 70
                typ = "SELECT_FIELD_FROM"
                if any(f in wh for f in requested):
                    s += 55
                    typ = "SELECT_FIELD_WHERE"
                if len(relevant) >= 2:
                    s += min((len(relevant)-1)*20, 60)
                    if any(f in wh for f in requested):
                        typ = "SELECT_FIELDS_WHERE"
                    else:
                        typ = "SELECT_FIELDS_FROM"
                if r["single"]:
                    s += 20
                # Explicit PA0001/PERNR relationship.
                if table == "PA0001" and "PERNR" in wh:
                    s += 45
                # Strong KOSTL lookup from PA0001 by PERNR.
                if table == "PA0001" and "KOSTL" in sel and "PERNR" in wh:
                    s += 55
                    typ = "SELECT_KOSTL_FROM_PA0001_WHERE_PERNR"
                # Organizational data from PA0001.
                org = {"BUKRS","WERKS","BTRTL","PERSG","PERSK","KOSTL","ORGEH","PLANS","STELL"}
                org_hit = sorted(org & sel)
                if table == "PA0001" and len(org_hit) >= 3:
                    s += min(len(org_hit)*12, 72)
                    if "PERNR" in wh or "PERNR" in sel:
                        s += 35
                        typ = "PA0001_ORG_FIELDS_PERNR"
                    else:
                        typ = "PA0001_ORG_FIELDS"
                consider(s, typ, self_reason(table, sel, wh, r["single"]))
            else:
                s = 45 + (15 if r["single"] else 0)
                consider(s, "TABLE_FROM", f"FROM {table} nello stesso SELECT")

        # Field-only queries: SAP prior is a modest tie-breaker only when the
        # candidate really uses the field with the prior table.
            ...
            for field in requested:
                for tab in FIELD_TABLE_PRIORS.get(field, []):
                    if table != tab:
                        continue
                    if field in sel:
                        s = 38 + (15 if r["single"] else 0)
                        if field in wh:
                            s += 18
                            typ = "SELECT_FIELD_WHERE"
                        else:
                            typ = "SELECT_FIELD_FROM"
                        consider(s, typ, f"SELECT {field} FROM {tab}")
                    elif field in wh:
                        consider(28, "SELECT_FIELD_WHERE", f"WHERE {field} su {tab}")

        # Qualified ABAP usage can prove a field/table relation without SQL.
        for field in requested:
            if target_tables:
...
def self_reason(table, sel, wh, single):
    ...
    if single:
        ...
        parts.append("SELECT")
    if sel:
        parts.append("fields=" + ",".join(sorted(sel)[:8]))
    parts.append("FROM " + table)
    if wh:
        parts.append("WHERE " + ",".join(sorted(wh)[:8]))
    return " | ".join(parts)
...
def relation_pass(x, p):
    ...
    if p["intent"] in {"code_lookup","code_pattern"} and "PA0001" in p["tables"]:
        return typ in {"SELECT_KOSTL_FROM_PA0001_WHERE_PERNR", "SELECT_FIELD_WHERE", "SELECT_FIELDS_WHERE", "SELECT_FROM"} and x.get("relationship_score",0) >= 90
    if p["intent"] == "functional_lookup" and "KOSTL" in p["functional_terms"]:
        return typ in {"SELECT_KOSTL_FROM_PA0001_WHERE_PERNR", "SELECT_FIELD_WHERE", "SELECT_FIELD_FROM", "SELECT_FIELDS_WHERE"} and x.get("relationship_score",0) >= 70
    if p["intent"] == "functional_lookup" and any(xc in p["concepts"] for xc in ("dati organizzativi", "dati organizzativi dipendente")):
        return typ in {"PA0001_ORG_FIELDS_PERNR", "PA0001_ORG_FIELDS", "SELECT_FIELDS_WHERE", "SELECT_FIELDS_FROM"} and x.get("relationship_score",0) >= 90
    if p["intent"] == "field_lookup":
        return typ in {"SELECT_FIELD_FROM","SELECT_FIELD_WHERE","FIELD_FROM"} and x.get("relationship_score",0) >= 35
    if p["intent"] == "table_reference":
        return typ in {"TABLE_FROM","SELECT_FIELD_FROM","SELECT_FIELDS_FROM","SELECT_FIELDS_WHERE"}
    return x.get("relationship_score",0) > 0
...
def rank(items, p):
    for x in items:
        ...
        # V5.5: relation score dominates for technical/code queries.
...
def main():
    ...
    OUTPUT_DIR.mkdir(parents=True,exist_ok=True)
    with (OUTPUT_DIR/"rag_v5_5_results.csv").open("w",newline="",encoding="utf-8-sig") as f:
        w=csv.DictWriter(f,fieldnames=rows[0].keys());w.writeheader();w.writerows(rows)

    md=[
        "# SAP ABAP RAG V5.5 - Report","",
    ...
    (OUTPUT_DIR/"rag_v5_5_report.md").write_text("\n".join(md),encoding="utf-8")
