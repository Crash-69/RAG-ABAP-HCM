...
def query_profile(q):
    u=q.upper()
    tables=[]; fields=[]
    for table, terms in SAP_TERMS.items():
        if re.search(rf"\b{table}\b",u):
            tables.append(table); fields.extend(terms)
    for table, terms in SAP_TERMS.items():
        for field in terms:
            if re.search(rf"\b{field}\b",u) and field not in fields:
                fields.append(field)
    if tables:
        intent = "code_pattern" if re.search(r"\bSELECT\b|\bWHERE\b|\bFROM\b",u) else "table_reference"
    elif re.search(r"\bSELECT\b|\bWHERE\b|\bFROM\b|\bleggere\b|\brecuperare\b|\bcodice\b",u):
        intent = "code_lookup"
    elif fields:
        intent = "field_lookup"
    else:
        intent = "functional_lookup"
    # Functional language maps common HCM concepts to likely PA0001 fields.
    if intent == "functional_lookup":
        mappings={
            "CENTRO DI COSTO":["KOSTL"], "DIPENDENTE":["PERNR"],
            "DATI ORGANIZZATIVI":["PERNR","WERKS","PERSG","PERSK","BTRTL","ORGEH","PLANS","STELL"],
            "ORGANIZZATIV":["WERKS","BTRTL","ORGEH","PLANS","STELL"]}
        for phrase, vals in mappings.items():
            if phrase in u:
                fields.extend(vals)
        fields=list(dict.fromkeys(fields))
        if any(x in fields for x in ("KOSTL","WERKS","BTRTL","ORGEH","PLANS","STELL","PERSG","PERSK")):
            tables=["PA0001"]
    return {"intent":intent,"tables":tables,"fields":list(dict.fromkeys(fields))}

def structural(content,tables,q,intent):
    t=content.upper(); score=0.; matches=[]
    weights={"table_reference":1.0,"field_lookup":0.45,"functional_lookup":0.55,"code_lookup":1.15,"code_pattern":1.30}
    w=weights.get(intent,1.0)
    for table in tables:
        n=len(re.findall(rf"\bFROM\s+{table}\b",t))
        if n: matches.append(f"FROM {table} ({n})"); score+=min(n,6)*8*w
        n=len(re.findall(rf"\bSELECT\b[\s\S]{{0,500}}?\bFROM\s+{table}\b",t))
        if n: matches.append(f"SELECT ... FROM {table} ({n})"); score+=min(n,4)*10*w
        n=len(re.findall(rf"\bSELECT\s+SINGLE\b[\s\S]{{0,500}}?\bFROM\s+{table}\b",t))
        if n: matches.append(f"SELECT SINGLE ... FROM {table} ({n})"); score+=min(n,4)*14*w
        n=len(re.findall(rf"\b{table}-[A-Z0-9_]+\b",t))
        if n: matches.append(f"{table}-FIELD ({n})"); score+=min(n,8)*3*w
    if re.search(r"\bWHERE\b[\s\S]{0,500}?\bPERNR\b",t): matches.append("WHERE ... PERNR"); score+=8*w
    if "KOSTL" in q.upper() and re.search(r"\bWHERE\b[\s\S]{0,500}?\bKOSTL\b",t): matches.append("WHERE ... KOSTL"); score+=8
    if re.search(r"\bLOOP\s+AT\b[\s\S]{0,200}?\bPA\d{4}\b",t): matches.append("LOOP AT PAxxxx"); score+=6*w
    if re.search(r"\bSELECT\b[\s\S]{0,700}?\bFROM\s+PA\d{4}\b",t) and re.search(r"\bWHERE\b[\s\S]{0,500}?\bPERNR\b",t): matches.append("SELECT TABLE + WHERE PERNR"); score+=10*w
    return score,matches

def rank(items,q,expected_tables,intent):
    for x in items:
        d=x.get("document"); m=d.metadata if d else x.get("metadata") or {}; c=d.page_content if d else x.get("content","")
        ss,sm=structural(c,expected_tables,q,intent); x["structural_score"]=ss; x["structural_matches"]=sm
...
def main():
    ...
    for no,case in enumerate(TEST_CASES,1):
        tid,q,intent,expected,tables=case; profile=query_profile(q); print("\n"+"="*70); print(f"TEST {no}/{len(TEST_CASES)} - {tid}\nQuery: {q}\nIntent: {intent}\nProfile: intent={profile["intent"]} tables={profile["tables"]} fields={profile["fields"]}")
        s,st=semantic(vdb,q); sem_total+=st; e=exact_search(cache,q); merged,dups=merge(s,e); final=rank(merged,q,profile["tables"],profile["intent"]); cov,th,ab,ss,se=evaluate(final,case)
