...
FINAL_K = 15
BATCH_SIZE = 500
OUTPUT_DIR = Path(r"C:\Progetto_AI\RAG_V5_7_RESULTS")
CACHE_FILE = OUTPUT_DIR / "exact_cache_v5_7.pkl"
CACHE_VERSION = "5.7"
...
def exact(cache, p):
    ...
    V5.7 treats exact SAP identifiers as first-class evidence. Compound queries
...
def relationship_score(c, p):
    """V5.7 intent-aware SQL relationship scoring."""
    text = c.upper()
...
def relation_pass(x, p):
    ...
    if p["intent"] in {"code_lookup", "code_pattern"} and "PA0001" in p["tables"]:
        return typ in {"SELECT_KOSTL_FROM_PA0001_WHERE_PERNR", "SELECT_FIELDS_WHERE", "SELECT_FIELD_WHERE", "PA0001_ORG_FIELDS_PERNR"} and rel >= 100
    if p["intent"] == "functional_lookup" and "KOSTL" in p["functional_terms"]:
...
def rank(items, p):
    ...
    for x in ordered:
        ...
        # V5.7: one representative per source family in the first page.
...
def main():
    ...
    OUTPUT_DIR.mkdir(parents=True,exist_ok=True)
    with (OUTPUT_DIR/"rag_v5_7_results.csv").open("w",newline="",encoding="utf-8-sig") as f:
        w=csv.DictWriter(f,fieldnames=rows[0].keys());w.writeheader();w.writerows(rows)

    md=[
        "# SAP ABAP RAG V5.7 - Report","",
    ...
    (OUTPUT_DIR/"rag_v5_7_report.md").write_text("\n".join(md),encoding="utf-8")
