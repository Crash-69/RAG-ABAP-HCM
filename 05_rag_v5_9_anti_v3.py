...
# SQL Relationship-Aware Ranking with Targeted T12 & T10/T11 Generalization
...
FINAL_K = 15
BATCH_SIZE = 500
OUTPUT_DIR = Path(r"C:\Progetto_AI\RAG_V5_9_ANTI_RESULTS")
CACHE_FILE = OUTPUT_DIR / "exact_cache_v5_9_anti.pkl"
OLD_CACHE_FILE = Path(r"C:\Progetto_AI\RAG_V5_9_RESULTS\exact_cache_v5_9.pkl")
CACHE_VERSION = "5.9_anti"
...
def load_cache(db):
    ...
    if CACHE_FILE.exists():
        try:
            ...
            if p.get("version") == CACHE_VERSION and p.get("total") == total:
                print(f"Cache V5.9 ANTI caricata: {len(p['items'])} chunk")
                return p["items"], 0
        except Exception:
            pass
    ...
    if OLD_CACHE_FILE.exists():
        try:
            p = pickle.loads(OLD_CACHE_FILE.read_bytes())
            if p.get("total") == total:
                print(f"Cache V5.9 originale ereditata: {len(p['items'])} chunk")
...
def exact(cache, p):
    ...
    for x in cache:
        content = x["content"]
        found = exact_terms(content, p["terms"])

        org_candidate = False
        org_fields_found = set()
        if (
            p["intent"] == "functional_lookup"
            and any(z in p["concepts"] for z in ("dati organizzativi", "dati organizzativi dipendente"))
        ):
            org = {"BUKRS","WERKS","BTRTL","PERSG","PERSK","KOSTL","ORGEH","PLANS","STELL"}
            relations_probe = extract_sql_relations(content)
            for r in relations_probe:
                if r["table"] == "PA0001":
                    org_fields_found |= org & (set(r["fields_select"]) | set(r["fields_where"]))
            org_candidate = len(org_fields_found) >= 2

        if not found and not org_candidate:
            continue
        ...
        if org_candidate:
            s += 14.0 * len(org_fields_found)
            if len(org_fields_found) >= 5:
                s += 40.0
            if len(org_fields_found) >= 7:
                s += 30.0
        ...
        for r in relations:
            ...
            target_set = set(p["tables"] + [t for t in SAP_TERMS if t in p["functional_terms"]])
            if table in target_set:
...
def relationship_score(c, p):
    """V5.9 ANTI SQL relationship scoring.

    Incorporates target table inference for functional queries, explicit scoring for
    organizational fields in same-SELECT statements (T12), and generalized PERNR
    filtering on PA0001 (T10/T11).
    """
    text = c.upper()
    ...
    inferred_tables = [t for t in SAP_TERMS if t in p["functional_terms"] or t in p["requested_fields"]]
    all_target_tables = list(dict.fromkeys(target_tables + inferred_tables))
    ...
    def consider(s, typ, reason):
        ...
        rank = {
            ...
            "SELECT_FIELDS_WHERE":6, "PA0001_ORG_FIELDS":8,
            "SELECT_PA0001_WHERE_PERNR":10, "SELECT_KOSTL_FROM_PA0001_WHERE_PERNR":10,
            "PA0001_ORG_FIELDS_PERNR":12,
        }.get(typ, 1)
        if s > score:
            score = s
    ...
    for r in relations:
        ...
        if table in all_target_tables:
            relevant_sel = set(requested) & sel
            relevant_where = set(requested) & wh

            if not relevant_sel and not relevant_where and table in target_tables:
                ...
                # T10 / T11 Generalization: SELECT <any fields> FROM PA0001 WHERE PERNR
                if table == "PA0001" and "PERNR" in wh:
                    s += 85
                    typ = "SELECT_PA0001_WHERE_PERNR"
                if table == "PA0001" and "KOSTL" in sel and "PERNR" in wh:
                    s += 100
                    typ = "SELECT_KOSTL_FROM_PA0001_WHERE_PERNR"

                # T12 Organizational fields scoring
                org_all = sorted(org & all_fields)
                if table == "PA0001" and len(org_all) >= 2:
                    org_score = len(org_all) * 22
                    if "PERNR" in all_fields:
                        org_score += 45

                    if p["intent"] == "functional_lookup" and any(z in p["concepts"] for z in ("dati organizzativi", "dati organizzativi dipendente")):
                        tot_rel_score = 130 + org_score
                        if "PERNR" in all_fields:
                            tot_rel_score += 55
                            consider(tot_rel_score, "PA0001_ORG_FIELDS_PERNR", f"SELECT {', '.join(org_all)} FROM PA0001 + WHERE PERNR ({len(org_all)} org fields)")
                        else:
                            consider(tot_rel_score, "PA0001_ORG_FIELDS", f"SELECT {', '.join(org_all)} FROM PA0001 ({len(org_all)} org fields)")
                    else:
                        if "PERNR" in all_fields:
                            consider(85 + org_score, "PA0001_ORG_FIELDS_PERNR", f"SELECT {', '.join(org_all)} FROM PA0001 + WHERE PERNR")
                        else:
                            consider(65 + org_score, "PA0001_ORG_FIELDS", f"SELECT {', '.join(org_all)} FROM PA0001")
...
def relationship_penalty_for_intent(x, p):
    ...
    if p["intent"] == "functional_lookup" and "KOSTL" in p["functional_terms"] and not any(z in p["concepts"] for z in ("dati organizzativi", "dati organizzativi dipendente")):
    ...
    if p["intent"] == "functional_lookup" and any(z in p["concepts"] for z in ("dati organizzativi", "dati organizzativi dipendente")):
        ...
        if typ == "TABLE_FROM":
            return 50.0
        if typ in {"SELECT_FROM", "SELECT_FIELD_FROM", "SELECT_FIELD_WHERE"} and not typ.startswith("PA0001_ORG"):
            return 45.0
...
def relation_pass(x, p):
    ...
    if p["intent"] in {"code_lookup", "code_pattern"} and "PA0001" in p["tables"]:
        return typ in {"SELECT_PA0001_WHERE_PERNR", "SELECT_KOSTL_FROM_PA0001_WHERE_PERNR", "SELECT_FIELDS_WHERE", "SELECT_FIELD_WHERE"} and rel >= 100
    if p["intent"] == "functional_lookup" and "KOSTL" in p["functional_terms"] and not any(z in p["concepts"] for z in ("dati organizzativi", "dati organizzativi dipendente")):
    ...
    if p["intent"] == "functional_lookup" and any(z in p["concepts"] for z in ("dati organizzativi", "dati organizzativi dipendente")):
        return typ in {"PA0001_ORG_FIELDS_PERNR", "PA0001_ORG_FIELDS"} and rel >= 110
...
def query_specificity_adjustment(x, p):
    ...
    if typ=="SELECT_PA0001_WHERE_PERNR":
        return 130.0, "GENERIC_PA0001_PERNR_BONUS"
    if typ=="SELECT_KOSTL_FROM_PA0001_WHERE_PERNR":
        return -130.0, "UNREQUESTED_KOSTL_SPECIALIZATION_PENALTY"
    return 0.0, ""
    ...
    for x in items:
        ...
        if p["intent"] == "functional_lookup" and any(z in p["concepts"] for z in ("dati organizzativi", "dati organizzativi dipendente")):
            ...
            if pa:
                ...
                has_p = any("PERNR" in (set(r["fields_select"]) | set(r["fields_where"])) for r in pa)
                score += min(best_n, 9) * 12
                if best_n >= 5:
                    score += 90
                elif best_n >= 3:
                    score += 40
...
def main():
    print("="*70)
    print("SAP ABAP RAG TEST - V5.9 ANTI V2")
    print("SQL RELATIONSHIP-AWARE RANKING WITH TARGETED T12 & T10/T11 FIXES")
    print("="*70)
    ...
    OUTPUT_DIR.mkdir(parents=True,exist_ok=True)
    with (OUTPUT_DIR/"rag_v5_9_anti_results.csv").open("w",newline="",encoding="utf-8-sig") as f:
        w=csv.DictWriter(f,fieldnames=rows[0].keys());w.writeheader();w.writerows(rows)

    md=[
        "# SAP ABAP RAG V5.9 ANTI - Report","",
    ...
    (OUTPUT_DIR/"rag_v5_9_anti_report.md").write_text("\n".join(md),encoding="utf-8")

    print("\n"+"="*70)
    print("SUMMARY V5.9 ANTI")
    print("="*70)
